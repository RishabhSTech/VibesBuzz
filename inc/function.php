...
class Actions{
    ...
    public function storeUseractions($data) {
        ...
        // Add new fields
        $your_gender = isset($data['your_gender']) ? mysqli_real_escape_string($conn, $data['your_gender']) : '';
        $your_age = isset($data['your_age']) ? mysqli_real_escape_string($conn, $data['your_age']) : '';

        $insertQuery = "INSERT INTO user_insta_profile (
            cap_name, cap_pincode, cap_whatsapp_num, 
            somethingabout, aboutcontent, storecontent, 
            mobile, user_name, totalviews, totalfollower, 
            profile_res, ReelsCount, loginusername, 
            insta_username, AccountCategory, totallikes2, 
            EarningAmount, last_post_date, avg_view_rate, 
            engagement_rate, postperweek, avgvideoduration, 
            totalcomment, your_gender, your_age
        ) VALUES (
            '$cap_name', '$cap_pincode', '$cap_whatsapp_num', 
            '$somethingabout', '$aboutcontent', '$storecontent', 
            '$mobile', '$user_name', '$totalviews', '$totalfollower', 
	...
	public function updateSurveydata($data) {
        ...
        $youtube_id = isset($data['youtube_id']) ? mysqli_real_escape_string($conn, $data['youtube_id']) : '';
        $your_caste = isset($data['your_caste']) ? mysqli_real_escape_string($conn, $data['your_caste']) : '';
